...
describe 'uploads data for collections', order: :defined do
  let(:collections)     { owner.all_owner_collections }
  let(:collection)      { collections.second }
  let(:set_collection)  { collections.last }
  let(:title)           { 'This is an empty work' }
  ...
  it 'starts a new project from tab', js: true do
    ...
    page.find('.tabs').click_link_or_button('Start A Project')
    page.find_by_id('document-upload').click
  ...
  it 'starts an ocr project', js: true do
    ...
    expect(uploaded_work.ocr_correction).to be true
  ...
  it 'imports IIIF manifests', js: true do
    ...
    VCR.use_cassette('iiif/imports_iiif_manifests', record: :new_episodes) do
      visit dashboard_owner_path
      page.find('.tabs').click_link_or_button('Start A Project')
      page.find_by_id('import-iiif-manifest').click
      ...
      click_link_or_button('Import Manifest')
  ...
  it 'creates an empty work', js: true do
    visit dashboard_owner_path
    page.find('.tabs').click_link_or_button('Start A Project')
    page.find_by_id('create-empty-work').click
    ...
    click_link_or_button('Create Work')
    expect(page).to have_content('Here you see the list of all pages in the work.')
    expect(Work.find_by(title: @title)).not_to be_nil
  end
  ...
  it 'adds pages to an empty work' do
    ...
    click_link_or_button('Save & New Work')
  ...
  it 'adds new document sets', js: true do
    ...
    page.find('.tabs').click_link_or_button('Settings')
    sleep 1

    page.find('.side-tabs').click_link_or_button('Look & Feel')
    page.check('Enable document sets')
    page.click_link_or_button('Edit Sets')
    ...
    expect(page).to have_current_path(collection_settings_path(@owner, DocumentSet.last))
  ...
  it 'adds works to document sets' do
    ...
    visit dashboard_owner_path
    page.find('.maincol').find('a', text: @set_collection.title).click
    page.find('.tabs').click_link_or_button('Sets')
