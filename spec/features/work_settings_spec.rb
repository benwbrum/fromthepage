...
describe 'Work Settings' do
  ...
  let(:work_no_ocr) { create(:work, owner_user_id: @owner.id, collection:, ocr_correction: false) }
  let(:page_no_ocr) { create(:page, work: work_no_ocr) }

  let(:work_ocr)   { create(:work, owner_user_id: @owner.id, collection:, ocr_correction: true) }
