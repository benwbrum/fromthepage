...
describe 'URL tests' do
  ...
  it 'edits a collection slug', js: true do
    ...
    expect(Collection.find_by(id: @collection.id).slug).to eq slug.to_s
    # test new path
  ...
  it 'edits a work slug' do
    ...
    expect(Work.find_by(id: @work.id).slug).to eq slug.to_s
    # test new path
  ...
  it 'edits a user slug' do
    ...
    expect(page).to have_content(@owner.display_name)
    expect(User.find_by(id: @owner.id).slug).to eq(slug.to_s)
    # test new path
    visit "/#{slug}"
    expect(page).to have_content(@owner.display_name)
    # test old path
