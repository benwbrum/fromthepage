...
describe 'collection settings js tasks', order: :defined do
  ...
  it 'sorts works in works list', js: true do
    ...
    expect(page.find('.collection-work-stats').find('tbody:nth-child(2)')).to have_content @collection.works.pick(:title)
    ...
    expect(page.find('.collection-work-stats').find('tbody:nth-child(2)')).to have_content @collection.works.order_by_completed.pick(:title)
    ...
    page.find('.collection-work-stats').find('thead').find('th', text: 'Most recent activity').click
    expect(page.find('.collection-work-stats').find('tbody:nth-child(2)')).to have_content @collection.works.order_by_recent_activity.pick(:title)
...
describe 'collection spec (isolated)' do
  ...
  after :all do
    @factory_owner.collections.each(&:destroy)
