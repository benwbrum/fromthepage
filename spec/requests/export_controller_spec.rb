  ...
  describe '#tei' do
    ...
    context 'with organization articles in descendant categories' do
      ...
      it 'omits empty organization category branches from taxonomy' do
        ...
        expect(response.body).not_to match(/<category xml:id=['"]C#{businesses_category.id}['"]>/)
        expect(response.body).not_to match(/<category xml:id=['"]C#{companies_category.id}['"]>/)
        expect(response.body).not_to match(/<category xml:id=['"]C#{military_units_category.id}['"]>/)
        expect(response.body).not_to match(/<category xml:id=['"]C#{regiments_category.id}['"]>/)
        expect(response.body).not_to match(/<category xml:id=['"]S#{business_article.id}['"]>/)
        expect(response.body).not_to match(/<category xml:id=['"]S#{military_unit_article.id}['"]>/)
      end
      ...
      it 'retains parent categories when a deeper descendant has a non-organization subject' do
        ...
        expect(response.body).to match(/<category xml:id=['"]C#{other_category.id}['"]>/)
        expect(response.body).to match(/<category xml:id=['"]C#{nested_other_category.id}['"]>/)
        expect(response.body).to match(/<category xml:id=['"]C#{deep_other_category.id}['"]>/)
        expect(response.body).to match(/<category xml:id=['"]S#{other_article.id}['"]>/)
      end
    end
