module SearchTranslator
  ...
  def self.search_canonical_from_xml(xml_transcript)
    ...
    all_titles = all_links.pluck('target_title')
    uniq_titles = all_titles.uniq

    all_abbrevs = []
    doc.xpath('//expan').each { |e| all_abbrevs << e['orig'] if e['orig'].present? }
  ...
  def self.strip_markup(xml_text)
    ...
    doc.xpath('//abbr').each { |n| n.replace(n['expan']) if n['expan'].present? }
    doc.xpath('//catchword').each(&:remove)
