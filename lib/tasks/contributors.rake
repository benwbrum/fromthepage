namespace :fromthepage do
  ...
  task :all_transcribers, [:collection_id] => :environment do |_t, args|
  ...
  task :recent_activity, [:collection_id] => :environment do |_t, args|
    collection_id = args.collection_id
    trans_deeds = DeedType.transcriptions_or_corrections
    ...
    if new_transcribers.empty?
      puts 'No new transcribers'
    else
      puts 'New Transcribers: <br> '
      new_transcribers.each do |t|
        puts "<a href='#{Rails.application.routes.url_helpers.url_for({
          controller: 'user',
          action: 'profile',
          user_id: t.id
        })}'>#{t.display_name}</a>  #{t.email} <br>"
      end
    end

    puts '<br>'

    if recent_transcriptions.empty?
