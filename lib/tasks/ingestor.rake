...
namespace :fromthepage do
  ...
  task :compress_images, [:pathname] => :environment do |_t, args|
  ...
  task :process_document_upload, [:document_upload_id] => :environment do |_t, args|
  ...
  def unzip_tree(temp_dir)
    ...
    ls.each do |path|
      ...
      elsif File.extname(path) == '.ZIP' || File.extname(path) == '.zip'
  ...
  def unpdf_tree(temp_dir, ocr)
    ...
    ls.each do |path|
      ...
      elsif File.extname(path) == '.PDF' || File.extname(path) == '.pdf'
  ...
  def compress_tree(temp_dir)
    ...
    ls.each do |path|
      ...
      if Dir.exist? path
        ...
        ImageHelper.compress_image(path)
      end
    end
  end
  ...
  def ingest_tree(document_upload, temp_dir)
    ...
    ls = Dir.glob(File.join(clean_dir, '*'))
    ls.each do |path|
  ...
  def convert_to_work(document_upload, path)
    ...
    new_dir_name = Rails.public_path.join('images', 'uploaded', work.id.to_s)
    ...
    IMAGE_FILE_EXTENSIONS.each do |ext|
      ...
      Dir.glob(File.join(clean_dir, "*.#{ext}")).each { |fn| print "\t\t\tcp #{fn} to #{new_dir_name}\n" }
  ...
  task :import_iiif, [:collection_url] => :environment do |_t, args|
