namespace :fromthepage do
  ...
  task :clean_bulk_exports, [:days_old] => :environment do |_t, args|
    days_old = args.days_old.to_i
    BulkExport.where(created_at: ...Time.now - days_old.days).each(&:clean_zip_file)
  end

  desc 'Process a bulk export'
  task :process_bulk_export, [:bulk_export_id] => :environment do |_t, args|
