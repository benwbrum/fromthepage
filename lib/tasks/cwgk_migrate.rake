namespace :fromthepage do
  namespace :cwgk do
    ...
    task :import_cwgk_xml, [:xml_directory, :collection_slug] => :environment do |t,args|
      ...
      id_to_title_map = {}
      valid_ids=[]
      # find or create a category with the title Geographic Feature
      ...
      if CREATE_SUBJECTS
        ...
        Dir.glob(File.join(xml_directory, "[NOPG]*.xml")).each do |file|
          ...
          if !article.save
            # this record has invalid attributes.  Add them to the list
            message = "Subject #{title} failed validation with these errors:\n"
            invalid_attrs = article.errors.keys
            invalid_attrs.each do |attr|
              message << "\t#{attr}:\t#{article[attr]}\t#{article.errors[attr]}\n"
              article[attr] = nil
            end
...
#          binding.pry unless article.valid?
      ...
      Dir.glob(File.join(xml_directory, "KYR*.xml")).sort.each do |file|
        ...
        file_contents = File.read(file)
        doc = Nokogiri::XML(file_contents)
        title_element=doc.search('titleStmt/title[type=main]')
        title_element=doc.search('title[type=main]') if title_element.blank?
        title_element=doc.search('title').first if title_element.blank?
        ...
        if !work.save
          # this record has invalid attributes.  Add them to the list
          message = "Work #{id}\t#{title} failed validation with these errors:\n"
          invalid_attrs = work.errors.keys
          invalid_attrs.each do |attr|
            message << "\t#{attr}:\t#{work[attr]}\t#{work.errors[attr]}\n"
            work[attr] = nil
          end
