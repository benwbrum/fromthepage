...
module SubjectExporter
  class Exporter
    ...
    def initialize(collection, works)
      ...
      @metadata_keys = collection.metadata_coverages.map(&:key)
    end
    ...
    def export
      CSV.generate(force_quotes: true) do |csv|
        ...
        collection.articles.includes(:categories).each do |article|
          ac_map[article] = article.categories.map(&:title).sort
        end
        ...
        @works.each do |work|
          ...
          work.pages.includes(:page_article_links, articles: [:page_article_links]).each do |page|
            ...
            sections_by_link, translation_sections, section_to_subjects = links_by_section(page.xml_translation, sections_by_link,
              translation_sections, section_to_subjects)
            ...
            page.page_article_links.each do |link|
              ...
              if article.nil?
                ...
                article_link = Rails.application.routes.url_helpers.collection_article_show_url(owner, collection, article.id,
                  only_path: false)
    ...
    def links_by_section(xml, links_hash, section_array, section_to_subjects = {})
      ...
      page.search('*').each do |e|
        ...
        when 'entryHeading'
          ...
          links = e.xpath('link')
          section_to_subjects[section_title] = links.pluck('target_title').join('||') if links.count > 0
