class WorkStatistic < ActiveRecord::Base
  ...
  def pct_annotated
    raw = (self[:annotated_pages].to_f - self[:needs_review].to_f) / self[:total_pages] * 100
    raw = 0 if raw.nan?
    [[0, raw].max, 100].min
  end

  def pct_corrected
    raw = (self[:corrected_pages].to_f - self[:needs_review].to_f + self[:blank_pages]) / self[:total_pages] * 100
    raw = 0 if raw.nan?
    [[0, raw].max, 100].min
  end
