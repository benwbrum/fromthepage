...
class SearchAttempt < ApplicationRecord
  ...
  def results_link
    ...
    case search_type
    when 'work'
      paths.paged_search_path(self, format: :html)
    when 'collection'
      paths.paged_search_path(self, format: :html)
    when 'collection-title'
      if collection.present?
        paths.collection_path(collection.owner, collection, search_attempt_id: id, format: :html)
      else # document_set
        paths.collection_path(document_set.owner, document_set, search_attempt_id: id, format: :html)
      end
    when 'findaproject'
      paths.search_attempt_show_path(self, format: :html)
    end
  end
