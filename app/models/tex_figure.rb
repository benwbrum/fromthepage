...
class TexFigure < ApplicationRecord
  ...
  def preprocess_latex
    FileUtils.rm_f(artifact_file_path)
    FileUtils.rm_f(raw_pdf_file_path)
    FileUtils.rm_f(cropped_pdf_file_path)
  end
  ...
  def run_latex
    latex_command = "#{XELATEX} -interaction batchmode -output-directory #{TexFigure.artifact_dir_name(page_id)} #{source_file_path}"
    logger.info(latex_command)
    puts latex_command
    puts `#{latex_command} 2>&1`
    $CHILD_STATUS # return the error code from running the command, not the return from puts
  end
  ...
  def postprocess_errors
    ...
    File.read(tex_log_file_path).scan(LATEX_ERROR).each do |text, line_id, line_no|
    ...
    EOF
    File.write(artifact_file_path, svg_string)
  end

  def write_source_file
    File.write(source_file_path, tex_source)
  end
  ...
  def review_artifact
    return unless changed.include? 'source'
  ...
  def artifact_file_path
    TexFigure.artifact_file_path(page.id, position)
  end
  ...
  def self.artifact_dir_name(page_id)
    Rails.public_path.join('images', 'working', 'tex', page_id.to_s)
  end

  def artifact_file_only
    artifact_file_only(position)
  end
