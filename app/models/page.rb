...
class Page < ApplicationRecord
    ...
    if COMPLETED_STATUSES.include? status
      ...
      if new_transcription.blank? && old_transcription.blank?
        ...
        self.approval_delta =
          ...
          (old_transcription.size + new_transcription.size)
      end
  ...
  def contributors
    ...
    page.page_versions.each do |cur_page|
      user = { name: cur_page.user.display_name }
      user[:orcid] = cur_page.user.orcid if cur_page.user.orcid.present?
  ...
  def generate_thumbnail
    ...
    factor = 400.to_f / self[:base_height]
