...
class Collection < ApplicationRecord
  ...
  def text_entry?
    data_entry_type == DataEntryType::TEXT_AND_METADATA || data_entry_type == DataEntryType::TEXT_ONLY
  end

  def metadata_entry?
    data_entry_type == DataEntryType::TEXT_AND_METADATA || data_entry_type == DataEntryType::METADATA_ONLY
  end

  def metadata_only_entry?
    data_entry_type == DataEntryType::METADATA_ONLY
  end

  def text_and_metadata_entry?
    data_entry_type == DataEntryType::TEXT_AND_METADATA
  end
  ...
  def pages_needing_review_for_one_off
    all_edits_by_user = deeds.where(deed_type: DeedType.transcriptions_or_corrections).group(:user_id).count
    one_off_editors = all_edits_by_user.select { |_k, v| v == 1 }.map { |k, _v| k }
    pages.where(status: Page::STATUS_NEEDS_REVIEW).joins(:current_version).where(page_versions: { user_id: one_off_editors })
  end

  def never_reviewed_users
    users_with_complete_pages = deeds.joins(:page).where('pages.status' => Page::COMPLETED_STATUSES).pluck(:user_id).uniq
    users_with_needs_review_pages = deeds.joins(:page).where('pages.status' => Page::STATUS_NEEDS_REVIEW).pluck(:user_id).uniq
    User.find(users_with_needs_review_pages - users_with_complete_pages)
  end
  ...
  def enable_messageboards
    if messageboard_group.nil?
      self.messageboard_group = Thredded::MessageboardGroup.create!(name: title)
      # now create the default messageboards
      Thredded::Messageboard.create!(name: 'General', description: 'General discussion', messageboard_group_id: messageboard_group.id)
      Thredded::Messageboard.create!(name: 'Help', messageboard_group_id: messageboard_group.id)
    end
  ...
  def show_to?(user)
    (!restricted && works.present? && collection_blocks.where(user_id: user&.id).none?
    ) || user&.like_owner?(self) || user&.collaborator?(self)
  end

  def create_categories
    # create two default categories
    category1 = Category.new(collection_id: id, title: 'People')
    category1.save
    category2 = Category.new(collection_id: id, title: 'Places')
    category2.save
  end
  ...
  def uniquify_slug
    return unless DocumentSet.exists?(slug:)

    self.slug = "#{slug}-collection"
  end

  def blank_out_collection
    puts "Reset all data in the #{title} collection to blank"
    works = Work.where(collection_id: id)
    ...
    Deed.where(article_id: articles.ids).destroy_all
    # delete articles
    Article.where(collection_id: id).destroy_all
    # delete categories (aside from the default)
    Category.where(collection_id: id).where.not(title: 'People').where.not(title: 'Places').destroy_all
    # delete notes
    ...
    pages.each do |p|
      ...
      p.update_columns(source_text: nil, created_on: Time.now, lock_version: 0, xml_text: nil, status: nil, source_translation: nil,
        xml_translation: nil, translation_status: nil, search_text: "\n\n\n\n")
      p.save!
    end
    ...
    puts "#{title} collection has been reset"
  end
  ...
  def sections
    Section.where(work_id: works.ids)
  end
  ...
  def set_transcription_conventions
    return if transcription_conventions.present?
...
ENDHELP
  ...
  def set_help
    return if help.present?

    self.help = DEFAULT_HELP_TEXT
  end

  def set_link_help
    return if link_help.present?
