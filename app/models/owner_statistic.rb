module OwnerStatistic
  ...
  def page_count
    Page.where(work_id: owner_works.ids).count
  end

  def active_page_count
    Collection.where(is_active: false).pluck(:id)
    Page.joins(work: :collection).
      where(work_id: owner_works.ids).
      where('collections.is_active': true).count
  end

  def incomplete_page_count
    Page.where(work_id: owner_works.ids).
  ...
  def review_count(days = nil)
    Deed.where(work_id: owner_works.ids).
      where(date_range_clause(days)).
      where(deed_type: DeedType::PAGE_REVIEWED).count
  end

  def owner_subjects
    Article.where(collection_id: all_owner_collections.ids)
  end

  def collection_ids
    [all_owner_collections.ids]
  end
  ...
  def all_owner_collections_updated_since(date_time_since)
    ...
    all_owner_collections & recently_changed
  end
  ...
  def new_collaborators_since(date_time_since)
    old_collaborators = User.joins(:deeds).
      ...
      where(deeds: { created_at: ...date_time_since }).distinct
    all_collaborators - old_collaborators
  end

  ## Helper functions for Owner Stats partial. TODO: Order by number of Deeds, scoped to this owner
  def editors_with_count
    contributor_deeds_by_type(DeedType::PAGE_EDIT, all_collaborators, collection_ids)
  end

  def transcribers_with_count
    contributor_deeds_by_type(DeedType::PAGE_TRANSCRIPTION, all_collaborators, collection_ids)
  end

  def indexers_with_count
    contributor_deeds_by_type(DeedType::PAGE_INDEXED, all_collaborators, collection_ids)
  end

  def reviewers_with_count
    contributor_deeds_by_type(DeedType::PAGE_REVIEWED, all_collaborators, collection_ids)
  end
  ...
  def contributor_deeds_by_type(deed_type, contributors, collections)
    deeds_by_user = Deed.group('user_id').where(collection_id: collections).where(deed_type:).order('count_id desc').count('id')
    deeds_by_user.map { |user_id, count| [contributors.find { |u| u.id == user_id }, count] }
  end
