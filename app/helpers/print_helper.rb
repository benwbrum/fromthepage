module PrintHelper
  ...
  def make_footnote_if_necessary(id, title, text)
    ...
    logger.debug "DEBUG: @printed_before[#{id}] == #{@printed_before[id]} (#{title})"
    ...
    begin
      ...
      if article.source_text.present?
    ...
    if title.length > MINIMUM_TITLE_TO_EXPAND && (title.length - text.length > TITLE_TO_TEXT_THRESHOLD)
