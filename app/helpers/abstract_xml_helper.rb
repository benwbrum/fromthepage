module AbstractXmlHelper
  ...
  def xml_to_html(xml_text, preserve_lb = true, flatten_links = false, collection = nil, highlight_article_id = nil)
    ...
    xml_text.gsub!("<lb break='no'/> ", '-<br />') if preserve_lb
        ...
        if id
          if flatten_links
            ...
            anchor.add_attribute('data-tooltip',
              url_for(controller: 'article', action: 'tooltip', article_id: id, collection_id: @collection.slug))
    ...
    doc.elements.each('//table') do |e|
      rend = e.attributes['rend']
      e.add_attribute('class', 'tabular') if rend == 'ruled'
    end
    ...
    if @page
      doc.elements.each('//texFigure') do |e|
        ...
        span.add_attribute('src', "#{file_to_url(TexFigure.artifact_file_path(@page.id, position))}?timestamp=#{Time.now.to_i}")
    ...
    my_display_html.gsub!(%r{</?page>}, '')

    ActionController::Base.helpers.sanitize(my_display_html.strip, tags: SANITIZE_ALLOWED_TAGS).gsub('<br>', '<br/>').gsub('<hr>',
      '<hr/>')
  end
