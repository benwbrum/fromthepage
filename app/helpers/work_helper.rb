module WorkHelper
  ...
  def xml_to_docbook(page)
    ...
    internal.elements.each('//link') do |e|
      ...
      if @displayed_already[article_id] || ((display_text.length - article_title.length).abs < 4)
        e.replace_with(REXML::Text.new(display_text.to_s))
      else
        # add the display text
        e.parent.insert_before(e, REXML::Text.new(display_text.to_s))
  ...
  def docbook_index_from_work(_work)
    ...
    sorted_articles.each do |article|
      next unless article.pages && article.pages.length > 0
