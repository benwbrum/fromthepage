module ApplicationHelper
  ...
  def billing_host
    if defined? BILLING_HOST
      ...
      session[:debug_billing] = true if params[:debug_billing]
      BILLING_HOST_DEVELOPMENT if session[:debug_billing] && defined? BILLING_HOST_DEVELOPMENT
    end
  end

  def html_block(tag)
    render({
      partial: 'page_block/html_block',
      locals: {
        tag:,
  ...
  def display_categories(categories, parent_id, expanded = false, &)
    ret = "<ul>\n"
    categories.each do |category|
      next unless category.parent_id == parent_id

      ret << "<li#{' class="expanded"' if expanded}>"
      ret << yield(category)
      ret << display_categories(category.children, category.id, expanded, &) if category.children.any?
      ret << '</li>'
    end
    ret << "</ul>\n"
  end
  ...
  def deeds_for(options = {})
    ...
    render({ partial: 'deed/deeds', locals: { limit:, deeds:, options: } })
  end
  ...
  def validation_summary(errors)
    return unless errors.is_a?(Enumerable) && errors.any?

    render({ partial: 'shared/validation_summary', locals: { errors: } })
  end
  ...
  def language_attrs(collection)
    ...
    attrs = {
      'lang' => display_language.to_s,
      'dir' => direction.to_s,
      'class' => direction.to_s
    }
  ...
  def value_to_html(value)
    if value.is_a? String
      ...
      value.pluck('@value').join('; ')

    elsif value.is_a? Hash
      # is this a pre-IIIF-v3 multi-language value?
      return value['@value'] if value.keys.include?('@language') && value.keys.include?('@value')

      value.values.map(&:first).join('<br/>')
  ...
  def html_metadata(metadata_hash)
    ...
    metadata_hash.each do |md|
      ...
      if label.is_a? Array
        ...
        label = label.values.map(&:first).join(' / ')
      end
      value = md['value']
  ...
  def target_collection_options(_default)
    option_data = {}
