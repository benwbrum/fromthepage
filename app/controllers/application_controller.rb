class ApplicationController < ActionController::Base
  ...
  def switch_locale(&)
    ...
    locale = session[:current_locale] if locale.nil? && session[:current_locale]
    ...
    if locale.nil? || I18n.available_locales.exclude?(locale.to_sym)
    ...
    related_locales = http_accept_language.user_preferred_languages.select do |loc|
      loc.to_s.include?(locale.to_s) && # is related to the chosen locale (is the locale, or is a regional version of it)
        I18n.available_locales.map(&:to_s).include?(loc.to_s) # is an available locale
    end
  ...
  def guest_user
    return if session[:guest_user_id].nil?
  ...
  def load_objects_from_params
    ...
    @collection = set_friendly_collection(params[:collection_id]) if params[:collection_id]
    ...
    @collection_ids = params[:collection_ids] if params[:collection_ids]
  ...
  def set_friendly_collection(id)
    ...
    if @work && (@work.collection != @collection) && !(@collection.is_a? DocumentSet)
  ...
  def authorize_collection
    ...
    if self.class.module_parent.name == 'Thredded' && !@collection.messageboards_enabled
    ...
    return if @collection.show_to?(current_user)
    # second chance?
    return if set_fallback_collection
  ...
  def set_fallback_collection
    if @work&.collection&.supports_document_sets
  ...
  def configure_permitted_parameters
    devise_parameter_sanitizer.permit(:sign_up) do |u|
      u.permit(:login, :email, :password, :password_confirmation, :display_name, :owner, :paid_date, :activity_email)
    end
    devise_parameter_sanitizer.permit(:sign_in) { |u| u.permit(:login_id, :login, :email, :password, :remember_me) }
    devise_parameter_sanitizer.permit(:account_update) do |u|
      u.permit(:login, :email, :password, :current_password, :password_confirmation, :real_name)
    end
  end
  ...
  def after_sign_in_path_for(_resource)
    if current_user.admin
      admin_path
    elsif session[:user_return_to].present? && session[:user_return_to] != '/' && session[:user_return_to].exclude?('/landing')
...
def check_api_access
  return unless (defined? @collection) && @collection
  return unless @collection.restricted && !@collection.api_access
  return unless @api_user.nil? || !@api_user.like_owner?(@collection)

  render status: :forbidden,
    plain: 'This collection is private.  The collection owner must enable API access to it or make it public for it to appear.'
end
...
def check_search_attempt
  return unless session[:search_attempt_id]

  your_profile = controller_name == 'user' && @user == current_user
  return unless ['dashboard', 'static'].include?(controller_name) || your_profile
...
def update_search_attempt_user(user, session_var)
  return unless session_var[:search_attempt_id]
