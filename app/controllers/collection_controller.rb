...
class CollectionController < ApplicationController
  ...
  before_action :review_authorized?,
    only: [:reviewer_dashboard, :works_to_review, :one_off_list, :recent_contributor_list, :user_contribution_list]
  ...
  def authorized?
    ajax_redirect_to dashboard_path unless user_signed_in?

    return false unless @collection && !current_user.like_owner?(@collection)
  ...
  def works_to_review
    @works = @collection.works.joins(:work_statistic).includes(:notes, :pages).
      where.not('work_statistics.needs_review' => 0).reorder('works.title').
  ...
  def approve_all
    ...
    @collection.works.each do |work|
      work.work_statistic&.recalculate({ type: Page::STATUS_NEEDS_REVIEW })
    end
    flash[:notice] = t('.approved_n_pages', page_count:)
    redirect_to(collection_quality_sampling_path(@collection.owner, @collection, @quality_sampling))
  end

  def edit_buttons
    @prefer_html = @collection.editor_buttons.exists?(prefer_html: true)
  end
  ...
  def update_buttons
    ...
    EditorButton::BUTTON_MAP.each_key do |key|
  ...
  def search
    @collection.metadata_coverages.where(key: params['facet_search']['label']).first
    ...
    @collection.works.each do |w|
      next if w.work_facet.nil?
      next unless years.include?(eval(year))
    ...
    @works = Work.joins(:work_facet).where(work_facets: { id: facet_ids }).paginate(page: params[:page], per_page: 10)
  ...
  def show
    if current_user && CollectionBlock.find_by(collection_id: @collection.id, user_id: current_user.id).present?
      ...
      if @collection.alphabetize_works
        ...
        order_clause = [
          'work_statistics.complete ASC',
          'work_statistics.transcribed_percentage ASC',
          'work_statistics.needs_review_percentage DESC'
        ].join(', ')
      end
      ...
      if @collection.nil?
        ...
        if @collection.restricted && (!user_signed_in? || !@collection.show_to?(current_user))
          flash[:error] = t('unauthorized_collection', project: @collection.title)
          redirect_to user_profile_path(@collection.owner)
        end
        ...
        if @collection.facets_enabled?
          ...
          unless params[:search].is_a?(String)
            @works = @collection.works.joins(:work_facet).where(work_facets: { id: facet_ids }).includes(:work_facet).paginate(
          ...
          if date_configs.size > 0
            ...
            date_configs.each do |facet_config|
              ...
              selection_values = @works.map { |w| w.work_facet.send(facet_attr) }.compact
              collection_values = collection_facets.map { |work_facet| work_facet.send(facet_attr) }.compact
          ...
          if params[:search_attempt_id]
            ...
            works = @collection.works.joins(:work_statistic).where(id: ids).reorder(order_clause).paginate(page: params[:page],
              per_page: 10)
  ...
  def add_owner
    ...
    send_email(@user, @collection) if @user.notification.add_as_owner
  ...
  def add_collaborator
    ...
    send_email(@user, @collection) if @user.notification.add_as_collaborator
  ...
  def edit_tasks
    ...
    return unless @collection.field_based && @collection.transcription_fields.blank?
  ...
  def create
    ...
    if @collection.save
      ...
      if request.referer.include?('sc_collections')
        session[:iiif_collection] = @collection.id
        ajax_redirect_to(request.referer)
      else
  ...
  def update
    ...
    if collection_params[:subjects_enabled].present?
      params[:collection][:subjects_disabled] = collection_params[:subjects_enabled] != '1'
    ...
    params[:collection][:slug] = @collection.title.parameterize if collection_params[:slug] == ''
    ...
    enable_fields if collection_params[:field_based] == '1' && !@collection.field_based
    ...
    if @collection.save
      ...
      edit_action = Rails.application.routes.recognize_path(request.referer)[:action]
  ...
  def works_list
    if params[:only_transcribed].present?
      @works = @collection.works.joins(:work_statistic).where(work_statistics: { transcribed_percentage: ...100 }).
        where(work_statistics: { needs_review: 0 }).order(:title)
    else
  ...
  def updated_fields_hash
    @collection.changed.index_with { |field| @collection.send(field) }
  end
  ...
  def load_settings
    ...
    @collaborators = @collection.collaborators.sort_by(&:display_name)
    @reviewers = @collection.reviewers.sort_by(&:display_name)
