...
class ExportController < ApplicationController
  ...
  layout proc { |controller| controller.request.xhr? ? false : nil } # , :only => [:update, :update_profile]
  ...
  def printable
    ...
    send_data(File.read(output_file), filename: File.basename(output_file), content_type:)
  ...
  def update_contentdm_credentials
    ...
    error_message, = ContentdmTranslator.fts_field_for_collection(@collection)
  ...
  def filtered_data
    ...
    @header = @collection.works.exists?(supports_translation: true) ? 'Translated' : 'Transcribed'
