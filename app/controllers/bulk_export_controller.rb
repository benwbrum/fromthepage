class BulkExportController < ApplicationController
  ...
  def create_for_work
    ...
    t('.export_running_message', email: current_user.email)
  ...
  def create_for_owner
    ...
    @bulk_export.user = current_user
    @bulk_export.status = BulkExport::Status::NEW
    @bulk_export.report_arguments = bulk_export_params[:report_arguments].to_h if bulk_export_params[:report_arguments]
  ...
  def create_for_work_actual
    ...
    return unless @bulk_export.save

    @bulk_export.submit_export_process

    flash.now[:info] = t('.export_running_message', email: current_user.email)
  end
