...
#    # spew string to docbook tempfile
  ...
  def versions
    @page_versions = 
      ...
      #PageVersion.find(:all, :joins => :page, :conditions => ['pages.work_id = ?', @work.id], :order => "work_version desc")
      PageVersion.joins(:page).where(['pages.work_id = ?', @work.id]).order("work_version desc").all
