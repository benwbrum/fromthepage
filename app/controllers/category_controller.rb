class CategoryController < ApplicationController
  ...
  layout proc { |controller| controller.request.xhr? ? false : nil }, only: [:edit, :add_new, :update, :create]
  ...
  def delete
    ...
    ajax_redirect_to collection_subjects_path(@collection.owner, @collection, { anchor: })
  end
  ...
  def enable_gis
    ...
    notice << " and #{count} child " << 'category'.pluralize(count) if count > 0
