class AdminController < ApplicationController
  ...
  def update_user
    ...
    if @user.update(user_params)
      if owner == false && @user.owner == true && SMTP_ENABLED
        begin
  ...
  def owner_list
    ...
    if params[:search]
      @owners = User.search(params[:search]).where(owner: true).order(paid_date: :desc).paginate(page: params[:page],
        per_page: PAGES_PER_SCREEN)
    elsif params[:sort]
      sort = params[:sort]
  ...
  def searches
    ...
    unless this_week.empty?
      ...
      @clickthrough_rate = (this_week.where('clicks > 0').count.fdiv(this_week.count) * 100).round(1)
      @clickthrough_rate_visit = (by_visit.sum(:clicks).values.count { |c| c > 0 }.fdiv(by_visit.length) * 100).round(1)
      @contribution_rate = (this_week.where('contributions > 0').count.fdiv(this_week.count) * 100).round(1)
      @contribution_rate_visit = (by_visit.sum(:contributions).values.count { |c| c > 0 }.fdiv(by_visit.length) * 100).round(1)
    end
