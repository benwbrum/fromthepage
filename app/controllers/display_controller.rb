class DisplayController < ApplicationController
  ...
  def read_work
    if params.key?(:work_id)
      @work = Work.friendly.find(params[:work_id])
    elsif params.key?(:id)
      @work = Work.friendly.find(params[:id])
    elsif params.key?(:url)
      @work = Work.find_by(id: params[:url][:work_id])
    end
    @review = params[:needs_review] if params.key?(:needs_review)
  ...
  def paged_search
    if @article
      ...
      @search_string = ''
      terms = @article.page_article_links.map do |link|
        link.display_text.gsub(/\s+/, ' ')
      end
      terms.uniq!
