class MasqueradesController < Devise::MasqueradesController
  ...
  def authorized?
    ...
    return false if user_signed_in? && admin_user.admin
  ...
  def show
    ...
    if Devise.masquerade_routes_back && Rails::VERSION::MAJOR == 5
      redirect_back(fallback_location: "#{after_masquerade_path_for(resource)}?#{after_masquerade_path_for(resource)}")
    elsif Devise.masquerade_routes_back && request.env['HTTP_REFERER'].present?
      redirect_back fallback_location: root_path
    else
      redirect_to("#{after_masquerade_path_for(resource)}?#{after_masquerade_path_for(resource)}")
    end
  end
