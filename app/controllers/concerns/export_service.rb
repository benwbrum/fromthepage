module ExportService
  ...
  def path_from_work(work, original_filenames = false)
    if original_filenames && work.uploaded_filename.present?
  ...
  def export_printable_to_zip(work, edition, output_format, out, _by_work, _original_filenames, preserve_lb, include_metadata,
  ...
  def export_owner_detailed_activity_csv(out:, owner:, report_arguments:)
    ...
    out.put_next_entry(path)
    out.write(detailed_activity_csv(owner, Time.zone.parse(report_arguments['start_date']), Time.zone.parse(report_arguments['end_date'])))
  end

  def export_admin_searches_csv(out:, report_arguments:)
    path = 'admin_searches.csv'
    out.put_next_entry(path)
    out.write(admin_searches_csv(Time.zone.parse(report_arguments['start_date']), Time.zone.parse(report_arguments['end_date'])))
  end

  def export_collection_activity_csv(out:, collection:, report_arguments:)
    path = 'collection_detailed_activity.csv'
    out.put_next_entry(path)
    out.write(collection_activity_csv(collection, Time.zone.parse(report_arguments['start_date']),
      Time.zone.parse(report_arguments['end_date'])))
  end

  def export_collection_contributors_csv(out:, collection:, report_arguments:)
    path = 'collection_contributors_activity.csv'
    out.put_next_entry(path)
    out.write(collection_contributors_csv(collection, Time.zone.parse(report_arguments['start_date']),
      Time.zone.parse(report_arguments['end_date'])))
  end
  ...
  def export_plaintext_translation(work:, name:, out:, by_work:, original_filenames:)
    ...
    return unless @work.supports_translation?
  ...
  def get_headings(collection, ids)
    ...
    renamed_cell_headings = TableCell.where(work_id: ids).where.not(transcription_field_id: nil).pluck(Arel.sql('DISTINCT header')) - collection.transcription_fields.pluck(:label)
    markdown_cell_headings = TableCell.where(work_id: ids).where(transcription_field_id: nil).pluck(Arel.sql('DISTINCT header'))
    ...
    field_headings.each do |field_id|
      ...
      if field && field.input_type == 'spreadsheet'
        ...
        field.spreadsheet_columns.each do |column|
          ...
          @headings << (collection.transcription_fields.present? ? raw_heading.to_s : "#{raw_heading} (text)")
          @headings << "#{raw_heading} (subject)" if collection.transcription_fields.blank?
        end
    ...
    CSV.generate(force_quotes: true) do |csv|
      ...
      collection.works.includes(:document_sets, :work_statistic, :sc_manifest).reorder(:id).each do |work|
        ...
        if work.metadata_description.present?
          ...
          # described by
          row << User.find(work.metadata_description_versions.pluck(:user_id)).map(&:display_name).join('; ')
  ...
  def generate_csv(work, csv, col_sections, transcription_field_flag, collection)
    ...
    if transcription_field_flag
      ...
      TableCell.where(work_id: work.id).where.not(transcription_field_id: nil).pluck(Arel.sql('DISTINCT header'))
      collection.transcription_fields.pluck(:label)
      input_types = collection.transcription_fields.pluck(:input_type)
    ...
    work.pages.includes(:table_cells).each do |page|
      next if page.table_cells.empty?

      has_spreadsheet = page.table_cells.detect do |cell|
        cell.transcription_field && cell.transcription_field.input_type == 'spreadsheet'
      end
      ...
      if page.sections.blank?
        if has_spreadsheet
          grouped_hash = {}
          spreadsheet_rows_and_ids = page.table_cells.where(transcription_field_id: spreadsheet_field_ids).pluck(
            :transcription_field_id, :row
          ).uniq
          spreadsheet_rows_and_ids.each_with_index do |field_id_and_row, i|
            ...
            grouped_hash[i + 1] = page.table_cells.where(row:, transcription_field_id:).to_a
          end
          grouped_hash[1] += page.table_cells.where.not(transcription_field_id: spreadsheet_field_ids).to_a
        else
        ...
        page.sections.each_with_index do |section, rownum|
          ...
          section.table_cells.group_by(&:row).each do |row, cell_array|
            ...
            running_data = process_header_footer_data(data_cells, running_data, cell_array, row) if has_spreadsheet
            # write the record to the CSV and start a new record
            csv << (page_cells + page_metadata_cells + section_cells + data_cells)
            # create a new array for the next row
            data_cells = Array.new(@headings.count, '')
  ...
  def page_metadata_cells(page)
    @page_metadata_headings.map do |key|
      page.metadata[key]
    end
  end
  ...
  def index_for_cell(cell)
    ...
    index ||= @indexable_headings.index(cell.header)
    index ||= @indexable_headings.index(cell.header.downcase)
    index ||= @indexable_headings.index(cell.header.strip.downcase)

    index
  end

  def cell_data(array, data_cells, transcription_field_flag, _count, _position, _spreadsheet_count)
    if transcription_field_flag
  ...
  def process_header_footer_data(data_cells, running_data, cell_array, count, position, spreadsheet_count, rownum)
    ...
    running_data = [] if running_data.nil?

    # are we in row 1?  fill the running data with non-spreadsheet fields
    if rownum == 1
      cell_array.each do |cell|
        next unless cell.transcription_field

        running_data << cell unless cell.transcription_field.input_type == 'spreadsheet'
      end
    else
  ...
  def collection_contributors_csv(collection, start_date, end_date)
    ...
    user_time_proportional = AhoyActivitySummary.where(collection_id: @collection.id,
      date: [start_date..end_date]).group(:user_id).sum(:minutes)
