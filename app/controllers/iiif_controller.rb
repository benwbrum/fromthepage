...
class IiifController < ApplicationController
  ...
  def load_objects_from_ids
    return unless @work && params[:work_id]
    return unless params[:work_id] =~ /^\d+$/
    return unless @work.id != params[:work_id].to_i
  ...
  def user_collections
    ...
    (@user.collections.to_a + @user.document_sets.to_a).each do |collection_or_ds|
      if collection_or_ds.is_public || collection_or_ds.api_access || @api_user&.like_owner?(collection_or_ds)
  ...
  def contributions
    ...
    if domain.blank?
      render status: :bad_request,
        plain: 'Usage: {url}/iiif/contributions/<b><i>domain</i></b>/<i>beginning of window</i>/<i>end of window</i><br />See <a href="https://github.com/benwbrum/fromthepage/wiki/FromThePage-Support-for-the-IIIF-Presentation-API-and-Web-Annotations">docs</a> for more help.'
      return
    end

    begin
      terminus_a_quo = Time.parse(raw_terminus_a_quo) if raw_terminus_a_quo
    rescue StandardError
      render status: :bad_request, plain: "Could not parse #{raw_terminus_a_quo} as a date. Try a format like #{Time.now.iso8601}"
      return
    end
    begin
      terminus_ad_quem = Time.parse(raw_terminus_ad_quem) if raw_terminus_ad_quem
    rescue StandardError
      render status: :bad_request, plain: "Could not parse #{raw_terminus_ad_quem} as a date. Try a format like #{Time.now.iso8601}"
      return
    end
  ...
  def for
    at_id = params[:id]
    at_id.sub!(':/', '://') if at_id.match(%r{https?:/\w})
    if (sc_collection = ScCollection.where(at_id:).last)
      redirect_to controller: 'iiif', action: 'collection', collection_id: sc_collection.collection_id
      return
    end

    if (sc_manifest = ScManifest.where(at_id:).last)
    ...
    if at_id.match(/http/)
      render status: :not_found,
        plain: "No items that correspond to #{at_id} have been imported into the FromThePage server.  For a full list of public IIIF resources, see #{url_for(
          controller: 'iiif', action: 'collections'
        )}"
    else
  ...
  def collection_for_domain(domain, terminus_a_quo = nil, terminus_ad_quem = nil)
    if domain.include? '.'
      ...
      if terminus_a_quo && terminus_ad_quem
        works = Work.joins(:work_statistic, :sc_manifest).where('sc_manifests.at_id LIKE ?',
          "%#{domain}%").where(work_statistic: { last_edit_at: terminus_a_quo..terminus_ad_quem }).distinct.includes(:work_statistic)
      elsif terminus_a_quo
        works = Work.joins(:work_statistic, :sc_manifest).where('sc_manifests.at_id LIKE ? AND work_statistics.last_edit_at >= ?',
          "%#{domain}%", terminus_a_quo).distinct.includes(:work_statistic)
      else
      ...
      if user&.owner
        if terminus_a_quo && terminus_ad_quem
          works = Work.joins(:collection, :work_statistic).where(collections: { owner_user_id: user.id }).where(work_statistic: { last_edit_at: terminus_a_quo..terminus_ad_quem }).distinct.includes(
            :work_statistic, :sc_manifest
          )
        elsif terminus_a_quo
          works = Work.joins(:collection, :work_statistic).where('collections.owner_user_id = ? AND work_statistics.last_edit_at >= ?', user.id, terminus_a_quo).distinct.includes(
            :work_statistic, :sc_manifest
          )
        else
  ...
  def manifest
    ...
    manifest.seeAlso << structured_data_reference(work) if work.collection.metadata_entry?
    manifest.service << status_service_for_manifest(work)
  ...
  def list
    ...
    annotation_list['@id'] =
      url_for({ controller: 'iiif', action: 'list', page_id: @page.id, annotation_type: type, only_path: false })
  ...
  def layer
    ...
    if params[:type] == 'transcription'
      ...
      work.pages.each do |page|
        ...
        layer['otherContent'] << annotation_list if annotation_list
      end
    end
  ...
  def structured_data_endpoint
    ...
    if @page && @page.notes.present?
      response['notes'] =
        url_for({ controller: 'iiif', action: 'list', page_id: @page.id, annotation_type: 'notes', only_path: false })
    end
    response['pageStatus'] = JSON.parse(status_service_for_page(@page).to_json) if @page
  ...
  def annotation
    ...
    annotation['@id'] =
      url_for({ controller: 'iiif', action: 'annotation', page_id: @page.id, annotation_type: type, only_path: false })
  ...
  def structured_data_label(_work, page = nil)
    if page
  ...
  def iiif_page_note(_page, noteid)
  ...
  def iiif_annotation_by_type(page_id, type)
    ...
    when 'transcription'
      ...
      annotation.resource = IIIF::Presentation::Resource.new({
        '@id' => collection_annotation_page_transcription_html_url(@work.owner, @collection, @work,
          @page).to_s,
    ...
    when 'translation'
      if page.source_translation.present?
        # annotation = IIIF::Presentation::Annotation.new
        # page = Page.find page_id
        annotation['on'] = region_from_page(@page)
        annotation.resource = IIIF::Presentation::Resource.new({
          '@id' => collection_annotation_page_translation_html_url(@work.owner, @collection, @work,
            @page).to_s,
          '@type' => 'cnt:ContentAsText'
        })
        annotation.resource['format'] = 'text/html'
  ...
  def iiif_image_annotation_from_work_id(_work_id)
    annotation
  end
  ...
  def iiif_sequence_from_work_id(work_id)
    ...
    sequence['@id'] =
      url_for({ controller: 'iiif', action: 'sequence', work_id:, sequence_name: 'default', only_path: false })
  ...
  def iiif_collection_from_collection(collection, depth)
    ...
    iiif_collection.metadata = [{ 'label' => 'dc:source', 'value' => collection.sc_collection.at_id }] if collection.sc_collection
  ...
  def dc_source_from_work(work)
    dc_source = nil
    if work.identifier.present? || work.sc_manifest || work.ia_work
  ...
  def canvas_from_page(page)
    return if page.base_width.nil? || page.base_height.nil?
  ...
  def add_annotations_to_canvas(canvas, page)
    if page.source_text.present?
      annotation_list = IIIF::Presentation::AnnotationList.new
      annotation_list['@id'] =
        url_for({ controller: 'iiif', action: 'list', page_id: page.id, annotation_type: 'transcription', only_path: false })
    ...
    if page.source_translation.present?
      annotation_list = IIIF::Presentation::AnnotationList.new
      annotation_list['@id'] =
        url_for({ controller: 'iiif', action: 'list', page_id: page.id, annotation_type: 'translation', only_path: false })
  ...
  def add_related_to_canvas(canvas, page)
    ...
    canvas.related << {
      ...
      'format' => 'text/html',
      '@id' => url_for(controller: :display, action: :display_page, page_id: page.id)
    }
    canvas.related << {
      'label' => 'Transcribe this page',
      'format' => 'text/html',
      '@id' => url_for(controller: :transcribe, action: :display_page, page_id: page.id)
    }
    return unless page.work.supports_translation?

    canvas.related << {
      'label' => 'Translate this page',
      'format' => 'text/html',
      '@id' => url_for(controller: :transcribe, action: :translate, page_id: page.id)
    }
  end
  ...
  def add_seeAlso_to_canvas(canvas, page)
    ...
    if page.work.supports_translation? && page.source_translation.present?
      canvas.seeAlso <<
        {
    ...
    return unless page.collection.field_based
  ...
  def annotationlist_from_page(page, type)
    ...
    when 'notes'
      if page.notes.present? # no comments
        annotation_list = IIIF::Presentation::AnnotationList.new
        annotation_list['@id'] =
          url_for({ controller: 'iiif', action: 'list', page_id: page.id, annotation_type: type, only_path: false })
        annotation_list['label'] = 'Notes'
  ...
  def status_service_for_manifest(work)
    ...
    service['pctTranscribed'] = work.ocr_correction ? 0.0 : stats.pct_completed
  ...
  def status_service_for_page(page)
    ...
    service['@context'] = 'http://www.fromthepage.org/jsonld/1/context.json'
    service['@id'] =
      url_for({ controller: 'iiif', action: 'canvas_status', work_id: page.work.id, page_id: page.id, only_path: false })
    service['pageStatus'] = []
    service['pageStatus'] << 'needsReview' if page.status == Page::STATUS_NEEDS_REVIEW
    if page.work.ocr_correction && (page.status == Page::STATUS_NEEDS_REVIEW || page.status == Page::STATUS_TRANSCRIBED)
      service['pageStatus'] << 'ocrCorrected'
    end
    service['pageStatus'] << 'markedBlank' if page.status == Page::STATUS_BLANK
    if page.status == Page::STATUS_NEEDS_REVIEW || page.status == Page::STATUS_TRANSCRIBED || page.status == Page::STATUS_INDEXED
      service['pageStatus'] << 'hasTranscript'
    end
    if page.translation_status == Page::STATUS_NEEDS_REVIEW || page.translation_status == Page::STATUS_TRANSLATED || page.translation_status == Page::STATUS_INDEXED
      service['pageStatus'] << 'hasTranslation'
    end
