class TitleController < ApplicationController
  ...
  before_filter :authorized?, :only => [:list]
  ...
  def list
    ...
    #@image_pages, @titled_images = paginate(:titled_image, {:per_page => 10, :conditions => conditions, :order => 'position' })
  end
  ...
  def renumber_images(change_by)
    ...
    for image in images
      if image.position >= start_position
        ...
        date += change_by
        #image.update_attributes({:title_seed => date.to_s, :title => date.strftime(date_format)})
        #TitledImage.transaction(image) do
        #  image.title_seed = date.to_s
        #  image.title = date.strftime(date_format)
        #  image.save!
        #end
        safe_update(image, { :title_seed => date.to_s, :title => date.strftime(date_format)})
      end
    end
