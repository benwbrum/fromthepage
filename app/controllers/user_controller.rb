class UserController < ApplicationController
  ...
  layout proc { |controller| controller.request.xhr? ? false : nil }, only: [:update, :update_profile, :api_key]
  ...
  def update
    ...
    if !@user.owner && (params[:user][:about] != NOTOWNER || params[:user][:about] != NOTOWNER)
      ...
      params_hash.delete_if { |_k, v| v == NOTOWNER }
  ...
  def update_profile
    ...
    @user.real_name = @user.display_name || @user.login if @user.real_name.blank?

    # Set dictation language to default (en-US) if it doesn't exist
    lang = @user.dictation_language.presence || 'en-US'
    ...
    @dialect_index = int.nil? ? nil : int - 2
  end
  ...
  def authorized?
    ...
    return false if current_user && (@user == current_user || current_user.admin?)

    redirect_to dashboard_path
  end

  def user_params
    params.require(:user).permit(:picture, :real_name, :orcid, :slug, :website, :location, :about, :preferred_locale, :help, :footer_block,
      notifications: [:user_activity, :owner_stats, :add_as_collaborator, :add_as_owner, :note_added, :add_as_reviewer])
  end
